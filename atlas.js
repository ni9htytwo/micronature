...
(function () {
  ...
  function draw() {
    ...
    if (G.mast) {
      ...
      lab([0.14, P.h + 0.12, zf], '传感器 ' + P.h.toFixed(2) + ' m', ink3, 'left', 3, -4);
    } else if (vis.length) {
      ...
      lab([ex + 0.28, yt, zf], '传感器 高 ' + P.h.toFixed(2) + ' m · 后方 ' + P.d.toFixed(2) + ' m',
          ink3, 'left', 3, -4);
    }
    ...
    if (missed) {
      ...
      lab([cx, top, zf], '判为地面连续', alarm, 'right', -7, -4, true);
    } else {
      seg([cx, -H * 0.5, zf], [cx, top, zf], acc, 1);
      lab([cx, top, zf], '沟被检出', acc, 'right', -7, -4, true);
    }

    ctx.fillStyle = ink2;
    ctx.font = '10px "IBM Plex Mono", ui-monospace, monospace';
    ctx.textAlign = 'left';
    ctx.fillText('真值 · 沟宽 0.45 m，深 0.80 m，剖切于光束所在断面', 16, hgt - 14);

    out.className = 'rig-out ' + (missed ? 'miss' : 'seen');
    vCount.innerHTML = '<b>' + S.inside + '</b>条落入沟内 · 成簇下限 ' + CLUSTER + ' 条';
    vCall.innerHTML = missed
      ? '<em>假阴性障碍</em>　沟内回波不足以成簇，被当作噪点滤除，机器判定地面连续，整台走进沟里。'
      : '<em>沟被检出</em>　沟内回波足以成簇，机器判定前方不可通行。';
  }
